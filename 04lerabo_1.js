window.addEventListener('DOMContentLoaded', () => {
    kakao.maps.load(() => {
        ...
        // 마커
        const leraboMarker = new kakao.maps.Marker({ position: leraboPosition, map });

        // 커스텀 오버레이
        const overlayContent = `
            <div class="customOverlay" style="background:#CFCFC9; padding:10px; border:1px solid #aaa; border-radius:8px; box-shadow:0 2px 6px rgba(0,0,0,0.3); position:relative; display:flex; align-items:center; gap:8px;">
                <img src="./source_03LE_LABO/logoS.jpg" alt="logo" style="width:50px;height:50px; border-radius:6px; flex-shrink:0;">
                <div style="font-size:1.4rem;">
                    <div style="position:relative; height:auto;">
                        <div style='font-weight:bold; font-size:1.6rem;'>LE RABO 플래그쉽 스토어</div>
                        <div style='background-color:#222; width:20rem; height:1px; margin:0.5rem auto;'></div>
                        <div style='margin-top:1rem auto;'>서울특별시 종로구 북촌로5나길 5</div>
        ...
        const overlay = new kakao.maps.CustomOverlay({
            ...
            position: new kakao.maps.LatLng(leraboPosition.getLat() + 0.0004, leraboPosition.getLng()),
            yAnchor: 1
        });
        overlay.setMap(map);

        // X 버튼 클릭 시 오버레이 제거
        const closeOverlayBtn = document.querySelector('.leraboMap .customOverlay .close-btn');
        closeOverlayBtn.addEventListener('click', () => overlay.setMap(null));

        // 마커 클릭 시 오버레이 다시 표시
        kakao.maps.event.addListener(leraboMarker, 'click', () => overlay.setMap(map));
        ...
        const storePositions = [
            new kakao.maps.LatLng(37.5796729, 126.9821614),
            new kakao.maps.LatLng(37.5814914, 126.9852635)
        ];
        const carparkPositions = [
            new kakao.maps.LatLng(37.5808564, 126.9816974),
            new kakao.maps.LatLng(37.5810398, 126.9829524)
        ];
